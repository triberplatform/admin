...
export async function GET(
  ...
  try {
    ...
    const businessData = await serverFetch(`/dealroom/business/received-proposals/${userId}`, {
      method: 'GET',
      headers: {
    ...
    console.error(`Error fetching proposals with ID ${params.id}:`, error);
    
    // Check for different error structures
    const errorMessage = 
      error.data?.message ||
      error.message ||
      `Failed to fetch proposals with ID ${params.id}`;
